...
def combine_img(configs):
  source = configs["args"]["source"]
  target = configs["args"]["target"]
  output = configs["args"]["output"]
  ...
  for mask in (segments.astype(np.uint8)):
    ...
    if configs["args"]["blend_type"] == "copy_paste":
      ...
      cv2.imwrite("./input/mask_tank.png", mask * 255)
      cat_img = blend(np.copy(tgt), np.copy(im), np.copy(mask))
      # cat_img = cv2.seamlessClone(im, tgt, mask * 255, (cd//2, rd//2), cv2.MIXED_CLONE)
    cv2.imwrite(f"{output}_{i}.png", cat_img)
    i+=1
